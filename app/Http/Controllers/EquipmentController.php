...
class EquipmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('equipments.index', compact('equipments', 'equipmentTypes', 'areas', 'companies', 'periodChecks'));
    }
    ...
    public function create()
    {
        ...
        $periodChecks = PeriodCheck::where('is_active', true)->get();
        return view('equipments.create', compact('equipmentTypes', 'companies', 'areas', 'locations', 'periodChecks'));
    }
    ...
    public function show(Equipment $equipment)
    {
        $equipment->load([
            ...
            'inspections' => function ($query) {
                $query->with('user')->orderBy('inspection_date', 'desc')->limit(5);
            }
        ]);
    ...
    public function edit(Equipment $equipment)
    {
        ...
        $locations = Location::where('is_active', true)->get();
        $periodChecks = PeriodCheck::where('is_active', true)->get();
        return view('equipments.edit', compact('equipment', 'equipmentTypes', 'companies', 'areas', 'locations', 'periodChecks'));
    }
    ...
    public function destroy(Equipment $equipment)
    {
        try {
            ...
            if ($e->getCode() == "23000") { // kode error integrity constraint violation
                return redirect()->route('equipments.index')
                    ->with('error', 'Equipment cannot be deleted because it still has inspection data.');
            }

            // fallback jika error lain
            return redirect()->route('equipments.index')
                ->with('error', 'An error occurred while deleting equipment.');
        }
    }
