...
@section('content')
    <div class="container">
        ...
        @endif
        <form method="POST" action="{{ route('headcrane.checksheetheadcrane.update', $checkSheetheadcrane->id) }}"
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="headcrane_number"
                            value="{{ $checkSheetheadcrane->headcrane->no_headcrane }}" name="headcrane_number" required
                            autofocus readonly>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-md-4">
                            <div class="custom-select"
                                ...
                                <p style="margin: 0; font-size: 1rem; color: #495057;">
                                    {{ $item->itemCheck->item_check }} - {{ $item->itemCheck->prosedur }}
                                </p>
                            </div>
                        </div>
                        <!-- Check Select -->
                        ...
                        <div class="col-md-2">
                            <select class="form-select" id="check" name="check[{{ $item->id }}]" required>
                                <option value="" disabled
                                    {{ old("check.$item->id", $item->check) === null ? 'selected' : '' }}>Select</option>
                                <option value="OK"
                                    {{ old("check.$item->id", $item->check) === 'OK' ? 'selected' : '' }}>OK</option>
                                <option value="NG"
                                    {{ old("check.$item->id", $item->check) === 'NG' ? 'selected' : '' }}>NG</option>
                            </select>
